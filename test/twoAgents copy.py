...
class QLearningAgent:
    def __init__(self, state_size, action_size, rewards, alpha=0.1, gamma=0.99, epsilon=0.5):
    ...
    def train(self, env, episodes, other_agent):
        joint_action_count = np.zeros((self.action_size, self.action_size))  # Contador de ações conjuntas
        for episode in range(episodes):
            state = env.reset()
            done = False
            while not done:
                action1 = self.choose_action(state)
                action2 = other_agent.choose_action(state)
                joint_action_count[action1][action2] += 1  # Contabilizando a ação conjunta
                next_state, reward1, reward2, done = env.step(state, action1, action2)
                self.update_q_table(state, action1, reward1, next_state)
                other_agent.update_q_table(state, action2, reward2, next_state)
                state = next_state
...
class PrisonersDilemmaEnv:
    def __init__(self):
        ...
        self.rewards = {
            ...
            1: {  # Recompensas para o estado 's1'
                (0, 0): [-1 ,  -1], 
...
if __name__ == "__main__":
    ...
    episodes = 1000
    joint_action_count = agent1.train(env, episodes, agent2)
